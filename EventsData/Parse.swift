  ...
  enum Router: URLRequestConvertible {
    ...
    var URLRequest: NSURLRequest {
      let (path: String, parameters: [String: AnyObject]) = {
        switch self {
        case .Users (let page):
          let params = ["myJavaScriptKey": Router.myJavaScriptKey, "page": "\(page)", "feature": "popular", "rpp": "50",  "include_store": "store_download", "include_states": "votes"]
          return ("/photos", params)
        case .PhotoInfo(let photoID, let imageSize):
          var params = ["myJavaScriptKey": Router.myJavaScriptKey, "image_size": "\(imageSize.rawValue)"]
          return ("/photos/\(photoID)", params)
        case .Comments(let photoID, let commentsPage):
          var params = ["myJavaScriptKey": Router.myJavaScriptKey, "comments": "1", "comments_page": "\(commentsPage)"]
